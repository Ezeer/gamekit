...
void gkGameLevel::tick(gkScalar delta)
{
	...
	//Here we could implement Network to test
		// starting with sending the state in a message
		//gkLogMessage("Message sent : STATE="<< m_player->getState());
		
	/*if(gkNetworkManager::getSingletonPtr()->isNetworkInstanceExists())
			if(gkNetworkManager::getSingletonPtr()->isNetworkInstanceRunning())
			{
				//gkPrintf("Message sent : STATE=%i",m_player->getState());
            gkNetworkManager::getSingletonPtr()->sendMessage("sender","receiver","stateUpdate","test of state networking");
			}*/
	}
